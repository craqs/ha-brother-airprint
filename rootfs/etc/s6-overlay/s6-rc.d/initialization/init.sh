...
# The container's default hostname doesn't resolve cross-subnet via router mDNS reflection,
# but the host's UUID hostname does. This makes DNS-SD SRV records point to a hostname
# that resolves on all subnets. publish-addresses=no avoids conflict with host's mDNS.
