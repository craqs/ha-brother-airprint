...
- Fix AirPrint cross-subnet on iPhone: use host Avahi instead of container Avahi
  - Container's own Avahi/dbus daemons removed; CUPS now registers with the HAOS host's Avahi via `host_dbus: true`
  - Printer advertised under `homeassistant.local` which resolves correctly on all devices and subnets
